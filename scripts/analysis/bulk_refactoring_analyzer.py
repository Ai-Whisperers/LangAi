...
class RefactoringReport:
    ...
    def generate_markdown(self) -> str:
        ...
        for pattern_type in sorted(by_type.keys()):
            ...
            lines.extend(
                [f"## {pattern_type}", "", f"**Total occurrences:** {len(findings_list)}", ""]
            )
            ...
            for i, finding in enumerate(findings_list, 1):
                ...
                if finding.notes:
                    lines.extend([f"**Notes:** {finding.notes}", ""])
                ...
                if finding.context_before or finding.context_after:
                    lines.extend(["<details>", "<summary>View context</summary>", "", "```python"])
...
class RefactoringAnalyzer:
    ...
    def _find_json_loads_patterns(self, file_path: Path, lines: List[str]):
        ...
        while i < len(lines):
            ...
            # Look for json.loads(
            if "json.loads(" in line and "safe_json_parse" not in "".join(
                lines[max(0, i - 5) : i + 1]
            ):
                ...
                if has_try_except:
                    for j in range(i + 1, min(len(lines), i + 15)):
                        if re.match(
                            r"\s*except\s+(json\.JSONDecodeError|Exception|ValueError)", lines[j]
                        ):
                            has_try_except = True
                            break
                    ...
                    if match:
                        ...
                        for j in range(i + 1, min(len(lines), i + 20)):
                            ...
                            if f"{var_name} =" in except_line or f"return " in except_line:
                                # Extract default
                                if (
                                    "return {}" in except_line
                                    or f"{var_name} = {{}}" in except_line
                                ):
                                    default_value = "{}"
                                elif (
                                    "return []" in except_line or f"{var_name} = []" in except_line
                                ):
                                    default_value = "[]"
                                break
        ...
        for i, line in enumerate(lines):
            ...
            if re.match(r"\s*except Exception as \w+:", line):
                ...
                specific_exceptions = self._suggest_specific_exceptions(
                    lines[max(0, i - 10) : i], context_after
                )

                if specific_exceptions:
                    suggested = line.replace(
                        "except Exception as", f"except ({', '.join(specific_exceptions)}) as"
                    )
    ...
    def _find_logging_getlogger(self, file_path: Path, lines: List[str]):
        ...
        for i, line in enumerate(lines):
            if re.search(r"logging\.getLogger\(__name__\)", line):
                ...
                file_content = "\n".join(lines)
                if (
                    "from company_researcher.utils import" in file_content
                    and "get_logger" in file_content
                ):
                    continue
                ...
                # Suggested pattern
                suggested = line.replace("logging.getLogger(__name__)", "get_logger(__name__)")
    ...
    def _find_datetime_patterns(self, file_path: Path, lines: List[str]):
        ...
        for i, line in enumerate(lines):
            ...
            if re.search(r"datetime\.(now\(timezone\.utc\)|utcnow\(\))", line):
                ...
                # Suggested pattern
                suggested = re.sub(
                    r"datetime\.(?:now\(timezone\.utc\)|utcnow\(\))", "utc_now()", line
                )

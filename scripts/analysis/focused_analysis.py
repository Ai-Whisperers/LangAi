...
class FocusedAnalyzer:
    ...
    def add_issue(self, file_path, line_number, error_type, description, severity):
        ...
        self.issue_count += 1
        rel_path = str(file_path.relative_to(self.root_path)) if isinstance(file_path, Path) else str(file_path)
        self.issues.append({
            'id': self.issue_count,
            'file': rel_path,
            'line': line_number,
    ...
    def check_missing_init(self):
        ...
        for dir_name in ['src', 'tests', 'scripts']:
            ...
            for subdir in dir_path.rglob('*'):
                ...
                if any(skip in str(subdir) for skip in ['__pycache__', '.git', 'External repos', '.venv', 'venv']):
                    continue
    ...
    def generate_report(self):
        ...
        print(f"\nBY TYPE:")
        print("-" * 40)
        for error_type, issues in sorted(by_type.items(), key=lambda x: len(x[1]), reverse=True)[:15]:

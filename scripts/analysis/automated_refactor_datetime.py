...
class DatetimeRefactorer:
    ...
    def refactor_file(self, file_path: Path) -> Tuple[bool, List[str]]:
        ...
        has_utc_now_import = bool(re.search(
            r'from company_researcher\.utils import.*\butc_now\b',
            content
        ))

        # Step 1: Replace datetime.now(timezone.utc) with utc_now()
        new_content, count = re.subn(
            r'\bdatetime\.now\(timezone\.utc\)',
            'utc_now()',
            content
        )

        if count > 0:
            changes.append(f"Replaced {count} occurrences of datetime.now(timezone.utc) with utc_now()")
    ...
    def _cleanup_datetime_imports(self, content: str) -> str:
        ...
        has_datetime_usage = bool(re.search(r'\bdatetime\.(datetime|timedelta|date|time)\b', content))
        ...
        if not has_datetime_usage and not has_timezone_usage:
            ...
            content = re.sub(
                r'from datetime import (.*?)timezone,?\s*',
                r'from datetime import \1',
                content
            )
            content = re.sub(
                r'from datetime import (.*?),\s*timezone',
                r'from datetime import \1',
                content
            )
...
def main():
    ...
    parser.add_argument(
        '--file',
        type=str,
        help='Refactor specific file only'
    )

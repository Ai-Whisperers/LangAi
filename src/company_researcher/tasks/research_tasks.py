...
if CELERY_AVAILABLE:
    ...
    def research_single_company(
        self, company_name: str, config: Optional[Dict] = None, save_to_db: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if repo:
                run = repo.create_research_run(
                    company_name=company_name, config_snapshot=config, triggered_by="celery_task"
                )
    ...
    def research_batch_companies(
        self, company_names: List[str], config: Optional[Dict] = None, parallel: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if parallel:
            ...
            job = group(research_single_company.s(name, config) for name in company_names)
    ...
    def update_company_data(
        company_name: str, force: bool = False, max_age_days: int = 7
    ) -> Dict[str, Any]:
        """
    ...
    def generate_report(
        research_run_id: str, formats: Optional[List[str]] = None, output_dir: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not run:
            return {"status": "failed", "error": f"Research run {research_run_id} not found"}
        ...
        report_data = {
            ...
            "agent_outputs": {o.agent_name: o.analysis for o in outputs},
        }
    ...
    def cleanup_old_runs(days_to_keep: int = 30) -> Dict[str, Any]:
        ...
        return {"status": "completed", "cutoff_date": cutoff.isoformat(), "runs_deleted": 0}
    ...
    def research_single_company(*args, **kwargs):
        raise ImportError(
            "Celery is required for async tasks. Install with: pip install celery redis"
        )

    ...
    for query in state["queries"][:5]:  # Limit to 5 queries for cost control
        try:
            ...
            if response.success and response.results:
                ...
                print(f"  [OK] '{query[:40]}...' via {response.provider}: {len(response.results)} results")
            else:
...
def website_scraping_node(state: OverallState) -> Dict[str, Any]:
    ...
    if JINA_AVAILABLE:
        ...
        for result in search_results:
            ...
            elif any(term in title for term in ["about us", "company", "who we are", company_name.lower()]):

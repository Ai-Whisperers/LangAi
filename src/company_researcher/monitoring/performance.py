...
class PerformanceMonitor:
    ...
    def record_latency(
        self, name: str, duration_seconds: float, context: Optional[Dict[str, Any]] = None
    ):
        """Record a latency measurement."""
        metric = PerformanceMetric(
            name=name, value=duration_seconds, unit="seconds", context=context or {}
        )
    ...
    def get_throughput(self, name: str = "api") -> ThroughputMetric:
        ...
        with self._lock:
            ...
            return ThroughputMetric(name=name, requests_per_second=rps, requests_per_minute=rpm)
    ...
    def _determine_level(self, name: str, p95: float) -> PerformanceLevel:
        ...
        thresholds = self._thresholds.get(
            name, {"excellent": 0.1, "good": 0.5, "acceptable": 1.0, "degraded": 5.0}
        )
    ...
    def set_thresholds(
        self, name: str, excellent: float, good: float, acceptable: float, degraded: float
    ):
    ...
    def _get_recommendation(self, name: str, summary: PerformanceSummary) -> str:
    ...
    def get_all_summaries(self) -> Dict[str, Dict[str, Any]]:
        ...
        with self._lock:
            return {name: self.get_summary(name).to_dict() for name in self._latencies.keys()}
    ...
    def cleanup(self, older_than: Optional[datetime] = None):
        ...
        with self._lock:
            for name in list(self._latencies.keys()):
                self._latencies[name] = [m for m in self._latencies[name] if m.timestamp >= cutoff]
...
class _MeasureContext:
    ...
    def __init__(self, monitor: PerformanceMonitor, name: str, context: Optional[Dict[str, Any]]):
...
def create_performance_monitor(
    window_size: int = 1000, retention_hours: int = 24
) -> PerformanceMonitor:
    """Create a performance monitor instance."""
    return PerformanceMonitor(window_size=window_size, retention_hours=retention_hours)

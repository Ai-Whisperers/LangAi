...
class SearchResultSentimentProfile(BaseModel):
    ...
    sentiment_score: float = Field(ge=-1.0, le=1.0, default=0.0, description="Overall sentiment score")
    sentiment_level: SentimentLevel = Field(default=SentimentLevel.NEUTRAL, description="Overall sentiment level")
    sentiment_trend: str = Field(default="stable", description="Trend direction: improving, stable, declining")
    ...
    negative_highlights: List[str] = Field(default_factory=list, description="Negative factors/concerns")

    # Category breakdown
    category_breakdown: Dict[str, int] = Field(
        default_factory=dict,
        description="Count of articles per category"
    )

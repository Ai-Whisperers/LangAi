...
class SearchResult:
    ...
    def calculate_combined_score(self, query: str):
        ...
        self.combined_score = (
            self.authority_score * 0.4 + self.relevance_score * 0.4 + rank_factor * 0.2
        )
    ...
    def _calculate_relevance(self, query: str) -> float:
        ...
        # Snippet match
        snippet_overlap = (
            len(query_terms & snippet_terms) / len(query_terms) if snippet_terms else 0
        )
...
class QueryDiversifier:
    ...
    def generate_diverse_queries(
        ...
        for category, config in QUERY_CATEGORIES.items():
            query_text = config["template"].format(company=company_name, year=year)
...
class RobustSearchClient:
    ...
    async def search_company(
        ...
        queries = self.diversifier.generate_diverse_queries(company_name, competitors, industry)
    ...
    async def _search_with_fallback(
        self, query: str, providers: List[str], max_results: int
    ) -> List[Dict]:
        """Search with fallback across providers."""
    ...
    async def _search_provider(self, provider: str, query: str, max_results: int) -> List[Dict]:
    ...
    def _generate_coverage_report(
        self, category_results: Dict[str, List[SearchResult]]
    ) -> Dict[str, Any]:
